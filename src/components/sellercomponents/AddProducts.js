...
export default function AddProducts(){
    ...
    const addProduct=async(e)=>{
       e.preventDefault()
           ...
           formData.set("img",imgs)
           formData.set("name",name)
           formData.set("price",price)
           formData.set("category",categorycode)
           formData.set('categoryTitle',category)
           formData.set("company",company)
           formData.set("userId",userId)
           formData.set("about",about)
           formData.set("subcat",subcategory)
           formData.set("subcatcod",subcategorycode)
            ...
            try{
              const response= await axios.post("add-product",formData)
                ...
                if(response.data.status===false){
                    setErrMsg("Product records not saved. Please try again.")}
                else if(response.status>499){
                    setErrMsg("Server is not working.. Try again later..")}
     ...
     const check=()=>{
          (name===""||imgs===""||price===""||subcategorycode===""||categorycode===""||company==="")?setErrMsg("Please enter all required fields"):setErrMsg("")
     }
    ...
    return(
           ...
           <OuterBox 
                ...
                {loader
                   ?<Loader/> 
                   :<MainBox theme={theme} sx={{backgroundColor:"white",boxShadow:"0px 0px 10px 0px black"}}>
                         ...
                         <Grid item xs={12} >
                            <Box sx={{display:"flex",overflow:"auto"}}>
                              {imgPreview.map((image, index) => (
                                <img style={{width:"100px",aspectRatio:1,padding:"10px"}} key={index} src={image} alt="Product Preview" />
                               ))}
                            </Box>
                         </Grid>
            
                         <Grid item xs={12}>
                             <Button1 type="submit" onClick={check} variant="contained" sx={{backgroundColor:"rgb(34, 22, 22)",":hover":{color:"white",backgroundColor:"rgb(53, 2, 2)"}}} fullWidth> Add Product </Button1>
                         </Grid>
                      </Grid>
                     </form>
                 </MainBox>}
