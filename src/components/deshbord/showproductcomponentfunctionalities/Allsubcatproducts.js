                               ...
                               return (
                                  <ThisProductCont key={index} xs={grdcnt} item >
                                     <ThisProductBox onClick={()=>props.comptype==="specific"?productDetails(item._id):navigate(`/identical-products/${item.name}`)}>
                                           <ProductHeading sx={{textAlign:"center",fontSize:{lg:"23px",md:"21px",sm:"18px",xs:"17px"}}}>{item.name}</ProductHeading>
                                           <ProductImage src={item.imgs[0].url} sx={{padding:"15px"}} alt=""/>
                                           <ProductDetailsBox>
                                               ...
                                               <Box sx={{display:"flex",flexDirection:"column",alignItems:"center",padding:"0px 15px 15px 15px"}}>  
                                                       <Details theme={theme}><i>{item.about&&item.about.length>100?`${item.about.slice(0,100)}...`:item.about.slice(0,100)}</i></Details><br/>
                                                       <Cost price={item.price} discount={item.discount}/>
