...
function BuyProduct() {
   ...
   useEffect(()=>{
          ...
          if(list===null){
                setreqfulfil(0)
                 ...
                 if(  list?.address[0]?.pincode
                       &&list?.address[0]?.Country
                       &&list?.address[0]?.City
                       &&list?.address[0]?.District
                       &&list?.address[0]?.State
                    ){
                       setreqfulfil(2)
                     }
 ...
 const getProduct=async()=>{
        ...
        setCharges(0);
        a&&a.map(item=>{
              settotalCost(pre=>pre+item.price-item.discount*item.price/100)
              setSaved(pre=>pre+savings(item))
              setCharges(pre=>pre+item.deliverCharges)
              return 0
         })
        setdata(a)
        setLoad(false)
 ...
 const dtl=async()=>{
        ...
        settotalCost(0);
        setSaved(0)
        settotalCost(a[0].price-a[0].discount*a[0].price/100)
        setSaved(savings(a[0]))
        setCharges(a[0].deliverCharges)
        setdata(a)
        setLoad(false)
 }
...
const ordradrs=(code)=>{
  setOrder({...order,OrdrAdrsId:code})
} 
...
const adrsupdt=(value)=>{
 ...
 const pricdtls=[
  ...
  {title:"Delivery Charges", icon:charges?<CurrencyRupeeIcon sx={{fontSize:"15px"}}/>:"", value:charges?charges:"FREE"},
  {title:<Divider/>,         icon:'',                                                     value:<Divider/>},
  {title:"Total Payable",    icon:<CurrencyRupeeIcon sx={{fontSize:"15px"}}/>,            value:totalcost+charges},
 ]

 const requirements=[
  {title:"LOGIN",           show1:<LogDfltComp email={list?.email}/>,                show2:<LogUpdtComp logUpdt={logUpdt}/>},
  {title:"DELIVERY ADDRESS",show1:<AdrsDfltComp  ordradrs={ordradrs} list={list}/>,  show2:<AdrsUpdtComp adrsupdt={adrsupdt} list={list}/>},
  ...
  return (
    ...
    {load
      ?<Loader/>
      :<>
         <Grid container sx={{minHeight:"60vh",position:"relative"}}>
         <ThisProductCont md={4} xs={12} item >
              <ThisProductBox  sx={{position:{md:'fixed',xs:"relative"}, zIndex:1,marginTop:{md:'400px'},left:{md:"320px"},width:{lg:"25%",md:"20%"}}}>
                    <ProductHeading sx={{textAlign:"center",padding:"5px",fontFamily:"b",fontSize:match?"15px":"20px"}}>
                      PRICE DETAILS
                    </ProductHeading>
                    <ProductDetailsBox>
                        <Grid container sx={{padding:"3px",margin:"20px 0px"}}>
                              ...
                              <ThisProductCont xs={12} item >
                                  <Typography sx={{fontWeight:600,color:"green",textAlign:"center"}}>
                  ...
                  <ThisProductBox>
                        <ProductHeading sx={{ display:"flex",justifyContent:"space-between",padding:"5px",fontFamily:"b",fontSize:match?"15px":"20px"}}>
                           <span>{index+1}. {item.title} 
                            ...
                            {(reqfulfil>index)&&editreq!==index&&<Button1 sx={{color:"white"}} onClick={()=>changelog(index)}>{index===1?"Add new":"Change"}</Button1>}
                        </ProductHeading>
