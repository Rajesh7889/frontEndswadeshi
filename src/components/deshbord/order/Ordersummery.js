...
import { savings} from '../../API/APIreq'
...
function Ordersummery({data,updtCost,updtsave,ordrsmry,email,history}) {
    ...
    const inc=(id,price,saving,discount)=>{
          setRecstat({...recstat,[id]:recstat[id]+1})
          updtCost(price-discount/100*price)
          updtsave(saving)
    }
  
    const dec=(id,price,saving,discount)=>{
      if(recstat[id]>1){
          setRecstat({...recstat,[id]:recstat[id]-1})
          updtCost(-price+discount/100*price)
          updtsave(-saving)}
          ...
          <Grid item xs={12} ><Typography sx={{paddingLeft:"5px"}}>Your order will be placed on <DeliveryDate/></Typography></Grid>
           {data?.length&&data.map((itm,index)=>{
            return(
                <Fragment key={index}>
                     <Grid item sm={4} xs={6}>
                         <ProductImage src={itm.imgs[0].url} sx={{maxHeight:"100px"}}></ProductImage>
                          <Box sx={{display:"flex",justifyContent:"center",alignItems:"center"}}>
                                 <CstmBtn onClick={()=>dec(itm._id,itm.price,savings(itm),itm.discount)}>-</CstmBtn>
                                 <ShowNmbr>{recstat[itm._id]}</ShowNmbr>
                                 <CstmBtn onClick={()=>inc(itm._id,itm.price,savings(itm),itm.discount)}>+</CstmBtn>
                          </Box>
                     </Grid>
                     <Grid item sm={8} xs={6}>
                         <Box sx={{padding:"5px"}}>
                            ...
                            <Cost price={itm.price*recstat[itm._id]} discount={itm.discount}/>
                          </Box>
                     </Grid>
...
export const CstmBtn=styled(Box)(()=>({
     display:"flex",
...
export const ShowNmbr=styled(Typography)(()=>({
  width:"30px",
  display:"flex",
  justifyContent:"center",
  alignItems:"center",
